...
DEFAULT_THRESHOLDS = {
    ...
    # 市值貨幣比的分區。
    #
    # 主要判準是**百分位**，不是絕對值。原因是這個數列有結構性的上升趨勢：
    # 2016 年中位 1.86、2024 年 2.81、2026 年 4.56——沒有任何一組固定數字能同時
    # 對十年前和今天說得通。原本那組（>4.5 吃緊、<2.5 寬鬆）是只有 5 個月歷史
    # 時訂的，補完 126 個月之後回頭看：2.5 以下涵蓋了 77% 的歷史，也就是說它把
    # 過去四分之三的月份都叫「寬鬆」；而 4.5 以上只在 2026 年出現過四個月。
    # 一個指標如果 77% 的時間都給同一個標籤，它就沒有在區分任何東西。
    #
    # 改成百分位之後，門檻會跟著歷史自己往上走，而「現在在自己歷史區間的哪裡」
    # 正是這個指標的說明文字一直在講的用法。
    "tw_marketcap_m1b_pct_high": 80,   # 落在歷史第 80 百分位以上 => 資金相對吃緊
    "tw_marketcap_m1b_pct_low": 20,    # 落在歷史第 20 百分位以下 => 資金相對寬鬆
    # 絕對值門檻只在歷史不足 24 個月時當備援（例如有人從零開始重建資料）。
    # 數字取自 126 個月的實際分布：p20 = 1.91、p80 = 2.60。
    "tw_marketcap_m1b_high": 2.6,
    "tw_marketcap_m1b_low": 1.9,
}
