...
追蹤台股、美股、日股三個市場的重要總經指標，以及一組日本焦點個股的股價與季度財報趨勢，
產生一份可以每天打開來看的 HTML 報告。

報告分成五個可收合的區塊，各有自己的色系：**重點摘要**、**台股重要經濟指標**、
**美股重要經濟指標**、**日股重要經濟指標**、**日股觀察**。

兩層都預設收合：打開報告看到的是五條橫幅，展開一塊之後裡面的卡片也還是收著的，
要看哪一張再點哪一張。
...
| 區塊 | 指標 | 來源 | 頻率 |
|---|---|---|---|
| 台股 | 加權指數、成交金額 | TWSE FMTQIK | 日 |
| 台股 | 製造業 PMI、非製造業 NMI | 國發會／中經院（data.gov.tw nid=6100） | 月 |
| 台股 | 市值貨幣比 | 自行計算：證期局 t49 總市值 ÷ 央行 M1B | 月 |
| 美股 | 道瓊、S&P 500、NASDAQ | Yahoo Finance | 日 |
| 美股 | GDP、零售銷售 | FRED `GDPC1`／`RSAFS` | 季／月 |
| 美股 | 非農就業、失業率、初請失業金 | FRED `PAYEMS`／`UNRATE`／`ICSA` | 月／週 |
| 美股 | CPI、核心 CPI、核心 PCE、PPI | FRED `CPIAUCSL` 等四項 | 月 |
| 美股 | 新屋開工、營建許可、密大信心 | FRED `HOUST`／`PERMIT`／`UMCSENT` | 月 |
| 美股 | VIX | CBOE 官方 CSV | 日 |
| 日股 | 日經 225、個股、村田 B/B Ratio | Yahoo Finance／人工登錄 | 日／季 |

FRED 的序列**不需要 API 金鑰**，換 `config.json` 的 `us_fred_series[].id` 就能換指標。

### 兩個要特別知道的限制

**ISM 製造業／服務業 PMI 沒有收錄。** ISM 在 2016 年因授權問題把序列從 FRED 下架
（`NAPM` 現在直接回 404），之後沒有免費、可程式化取得的官方管道。與其放推估值或
來路不明的數字，這裡選擇不放——這是刻意的取捨，不是漏做。想看數字請直接查
ISM 官網的當期新聞稿。

**市值貨幣比會落後好幾個月。** 分子的總市值來自金管會證期局，公布時間「不定期」，
而且端點只回最近 5 個月的滾動視窗（試過各種分頁參數，回傳位元組完全一樣）。所以：

- 歷史只能靠每天跑的時候把新月份**併進**本地檔案慢慢累積
- 報告上標的是「資料月份 YYYY/MM」而不是「最新」，落後三個月以上會轉成橘色

---

## 說明文字（燈泡）

報告裡每個指標旁邊的 💡 說明，內容全部集中在 **`explanations.py`** 的 `TIPS`：

```python
TIPS["tw_pmi"] = ("PMI／NMI 怎麼看", """50 是榮枯線……""")
```

想改一段解釋、換成自己的寫法、或加一個新指標的說明，只要動這個檔，
不用碰 `generate_report_local.py` 那一千多行的排版邏輯。
表格「核心指標速覽表」的內容則在同一個檔的 `OVERVIEW_TABLE`。
...
總經指標同樣會進頁首摘要：PMI 跌破 50 榮枯線、市值貨幣比高於
`tw_marketcap_m1b_high`（預設 4.5）或低於 `tw_marketcap_m1b_low`（預設 2.5）、
VIX 與密大信心觸線時都會列出來。市值貨幣比那一條一定會帶上資料月份，
因為它本來就落後，不標清楚容易被誤讀成「現在」。
...
├── fetch_market_data.py    抓價格與總經指標資料
├── generate_report_local.py 產生報告
├── explanations.py         報告裡所有燈泡說明文字與速覽表內容
...
python tests/test_parsers_offline.py   跑離線 parser 測試（不打網路，幾秒鐘）
```
...
- 美股總經：FRED 官方 CSV，免金鑰；VIX 走 CBOE 官方 CSV（比 FRED 那份即時）
- 台股總經：政府資料開放平臺（國發會 PMI）、中央銀行（M1B）、金管會證期局（總市值）
- 財報：人工登錄自各公司官方決算短信，來源與科目定義記在每個資料檔的 `methodology` 欄位
- 報告不會編造數字。缺漏的資料就是留白，不會用推估值填補

### 抓取失敗時會怎樣

`fetch_market_data.py` 的結束碼分成三種，**不是**「失敗就整個中斷」：

| 結束碼 | 意思 | 後果 |
|---|---|---|
| 0 | 全部成功 | 正常產生報告 |
| 2 | 有來源失敗，但既有快取還在 | **照樣產生報告**，Actions 留一個警告註記 |
| 1 | 致命錯誤 | 中斷，不產生報告 |

會這樣設計是因為官方端點偶爾抽風是常態。如果一個端點 404 就讓當天完全沒有報告，
那比靜靜失敗更糟；但也絕不能安靜帶過，所以失敗清單會列在輸出裡、在 Actions 上留警告，
連續幾天出現就看得出來。

另外有一類「軟失敗」會單獨列出：來源沒抓到但既有快取可用。這種情況報告看起來完全正常，
只是那格的資料月份不會前進——最容易被忽略，所以特別標出來。

### 端點原始回應（fixture）

`reference/samples/` 存的是每個端點**真的打過**拿回來的原始 bytes
（`*.raw.gz` ＋ `*.meta.json`，gzip 用 `mtime=0` 壓成可 diff 的形式）。
`tests/test_parsers_offline.py` 直接拿它們餵 parser，所以端點哪天改格式，
測試會在錯誤資料進到報告之前先紅。

**沒用的端點也留著**，並在 meta 的 `note` 寫明為什麼不是它——
不留的話，六個月後會有人再去試一次同一個 URL。目前記錄在案的有：
央行 `EF19M01`（欄名幾乎相同但值是「當月變動額」不是餘額）、
證期局 `t32`（單位是美元且不含上櫃）、
TWSE `MI_INDEX`（9 張表 59 個欄位，確認沒有市值）。
